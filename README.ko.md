...
- sample count, hot field-order 안정성, payload size, drift risk로 fast-path 후보를 랭킹합니다.
...
- variable-length list shape용 homogeneous-array skeleton fingerprint를 계산합니다.
...
| Candidate report | fast-path 후보 scoring과 sample Gradle report task. |
| Shape guard | exact matcher, fingerprint matcher, checkpointed early rejection. |
| Skeleton fingerprint | variable-length homogeneous array가 shape fingerprint를 공유할 수 있습니다. |
...
captured sample file은 한 줄에 하나의 JSON payload를 둘 수 있습니다.
...
`candidateSamples`는 `.json` 파일 directory를 가리킬 수도 있습니다. 파일 경로는
endpoint 이름으로 변환되고, `orders/create-1.json` 같은 숫자 suffix는
`/orders/create`로 묶입니다.

선택적인 candidate config는 Java `.properties` 문법을 사용합니다.
...
`endpoint.<prefix>=<target>`은 captured endpoint 이름을 scoring 전에 묶습니다.
`redactFields`는 profiling 전에 일치하는 top-level JSON field 값을 `null`로 바꿔
민감한 scalar 값이 report에 영향을 주지 않게 합니다.

CI나 다른 도구가 report를 읽어야 한다면 `-PcandidateOutput=json`을 사용합니다.
`compareFastPathCandidateReports`는 current report의 score 급락, hot-order 안정성 하락,
dropped-order 증가가 크면 CI를 실패시키는 데 사용할 수 있습니다.
