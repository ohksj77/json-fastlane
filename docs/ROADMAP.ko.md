...
| Fast-path candidate report | `JsonFastlaneReport.candidates`가 endpoint를 scoring하고 `fastPathCandidateReport`가 endpoint mapping, field redaction, text/JSON output을 적용해 synthetic, TSV, directory sample을 읽습니다. |
| Candidate regression gate | `compareFastPathCandidateReports`가 JSON report를 비교하고 score, stability, dropped-order regression이 있으면 실패합니다. |
...
| Skeleton fingerprint | homogeneous-array skeleton hash가 variable-length list shape discovery를 지원합니다. |
...
| Homogeneous arrays | skeleton fingerprint가 discovery 단계에서 variable-length list shape를 정규화합니다. | routing에는 여전히 exact verification 또는 generated reader fallback 동작이 필요합니다. |

## 다음 깊은 작업

1. retained shape에서 `TryFastJsonReader` 구현을 생성합니다.
2. captured-sample candidate report를 더 풍부한 endpoint mapping, 안전한 redaction, CI artifact용 regression policy를 가진 production-facing CLI로 확장합니다.
3. boxed value, enum, nested record, nullable field, collection, 명확한 unsupported-feature
   diagnostics까지 processor coverage를 넓힙니다.
4. list가 많은 endpoint ranking이 더 정확해지도록 skeleton fingerprint를 candidate scoring에 사용합니다.
5. transport benchmark를 large array와 optional-field shape가 섞인 조건까지 확장합니다.
6. Netty writer registry 위에 WebFlux codec을 추가합니다.
7. generated DTO에는 `ObjectMapper`에 가까운 사용감을 주고 unsupported type은 fallback하는
   optional facade를 설계합니다.
8. naming strategy, null policy, enum policy, date/time, custom serializer,
